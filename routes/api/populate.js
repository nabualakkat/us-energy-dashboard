...
router.get('/', async (req, res) => {
  ...
  //   const industrialData = await axios.get(`${baseURI}&series_id=TOTAL.TEICBUS.M`)
  //   const industrialSector = industrialData.data.series[0]
  //   const residentialData = await axios.get(`${baseURI}&series_id=TOTAL.TERCBUS.M`)
  //   const residentialSector = residentialData.data.series[0]
  //   const transportationData = await axios.get(`${baseURI}&series_id=TOTAL.TEACBUS.M`)
  ...
  //       ...totalEnergyConsumedBySector,
  //       {
  //         month: moment(item[0], 'YYYYMM'),
  //         data: {
  //         commercialSector: item[1],
  //         industrialSector: industrialSector.data[i][1],
  //         residentialSector: residentialSector.data[i][1],
  //         transportationSector: transportationSector.data[i][1],
  //       }}
  //     ]
  //   })
  //   await ConsumptionBySector.collection.insertMany(totalEnergyConsumedBySector)
  //   res.status(200).send()
  // } catch (e) {
...
//       const consumptionData = data.consumptionUri !== data.productionUri ? await axios.get(data.consumptionUri) : productionData
//       const consumption = consumptionData.data.series[0]
//       return {[data.name]:{production: production.data, consumption: consumption.data}}
//   }
//   const getData = async () => {
...
//                 consumption: dataObject.hydroelectricProduction.consumption[i][1]
//               },
//               naturalGas: {
...
//         ]
//       })
//       ProductionBySource.collection.insertMany(netProductionBySource).then(() => res.status(200).send())
