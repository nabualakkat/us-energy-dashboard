...
router.get('/emissions', async (req, res) => {
  try {
    const emissionsData = await axios.get(
      `${baseURI}&series_id=TOTAL.TETCEUS.A&num=10`
    );
...
router.get('/expenditure', async (req, res) => {
  try {
    const averageRetailPriceData = await axios.get(
      `${baseURI}&series_id=TOTAL.ESTCUUS.A&num=10`
    );
    ...
    const energyExpendituresData = await axios.get(
      `${baseURI}&series_id=TOTAL.TETCVUS.A&num=10`
    );
    ...
    Expenditure.collection
      .insertMany([...averageRetailPrice, ...energyExpenditures])
      .then(() => res.status(200).send());
  } catch (e) {
    console.error(e.message);
    process.exit(1);
